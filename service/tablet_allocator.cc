...
class load_balancer {
    ...
    utils::unordered_map<table_id, uint64_t> _disk_used_per_dc_per_table;
    // Holds total used storage per table in the DC/rack
    absl::flat_hash_map<std::pair<dc_name, sstring>, utils::unordered_map<table_id, uint64_t>> _disk_used_per_rack_per_table;
    dc_name _dc;
    ...
    future<migration_plan> make_plan(dc_name dc) {
        ...
        for (auto table : std::views::keys(_tm->tablets().all_table_groups())) {
            ...
            for (const auto& [host, n] : nodes) {
                ...
                size_t node_tablet_count = 0;
                if (n.tablet_sizes_per_table.contains(table)) {
                    node_du.used = n.tablet_sizes_per_table.at(table);
                    node_tablet_count = n.tablet_count_per_table.at(table);
                }
                lblogger.debug(" node: {} tablets: {} load: {}", n.id, node_tablet_count, node_du);
                for (shard_id sid = 0; sid < n.shards.size(); sid++) {
                    ...
                    size_t shard_tablet_count = 0;
                    if (shard_load.tablet_sizes_per_table.contains(table)) {
                        shard_du.used = shard_load.tablet_sizes_per_table.at(table);
                        shard_tablet_count = shard_load.tablet_count_per_table.at(table);
                    }
                    lblogger.debug("  shard: {} tablets: {} load: {}", sid, shard_tablet_count, shard_du);
                }
            }
        }
    ...
    for (const auto& [host, tls] : tablet_stats) {
        ...
        if (log_tablet_sizes) {
            ...
            for (const auto& [rb_tid, size] : tls.tablet_sizes) {
                service::lblogger.info("  tablet: {} size: {} raw_size: {}", rb_tid, locator::bytes2gb(size), size);
            }
        }
    }
    service::lblogger.info("-- load_stats tables {}", reason);
    for (const auto& [table, table_s] : tables) {
        service::lblogger.info(" table: {} raw_size: {}", table, table_s.size_in_bytes);
    }
}
