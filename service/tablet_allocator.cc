...
public:
    ...
    future<migration_plan> make_plan(dc_name dc) {
        ...
        auto ensure_node = [&, this] (host_id host) {
            if (nodes.contains(host)) {
                return;
            }
            auto* node = topo.find_node(host);
...
public:
    ...
    future<migration_plan> balance_tablets(token_metadata_ptr tm, locator::load_stats_ptr table_load_stats, std::unordered_set<host_id> skiplist, locator::disk_usage_by_host disk_usage) {
...
future<migration_plan> tablet_allocator::balance_tablets(locator::token_metadata_ptr tm, locator::load_stats_ptr load_stats, std::unordered_set<host_id> skiplist,
    ...
    return impl().balance_tablets(std::move(tm), std::move(load_stats), std::move(skiplist), std::move(disk_usage));
}
