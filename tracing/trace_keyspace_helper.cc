...
future<> trace_keyspace_helper::start(cql3::query_processor& qp, service::migration_manager& mm) {
    ...
    if (qp.db().features().auto_replication_factor) {
        ...
        strategy = "org.apache.cassandra.locator.NetworkTopologyStrategy";
    }

    return table_helper::setup_keyspace(qp, mm, KEYSPACE_NAME, std::move(strategy), rf, _dummy_query_state, { &_sessions, &_sessions_time_idx, &_events, &_slow_query_log, &_slow_query_log_time_idx }, initial_tablets, per_table_tablet_options);
}
