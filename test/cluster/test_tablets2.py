...
async def test_tablet_cleanup_failure(manager: ManagerClient):
    ...
    # Make sure both nodes see the same disk capacity, so that after the move_tablet API
    # the load balancer does not attempt to migrate the tablet back to the first node.
    # Without this, the second node will see a slightly lower disk capacity, the load
    # balancer will compute a higher load, and move the tablet back to the first node.
    config = { 'data_file_capacity': 100 * 1024 * 1024 }
