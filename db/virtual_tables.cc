...
public:
    ...
    future<> execute_on_leader(std::function<void(mutation)> mutation_sink, reader_permit permit) override {
        ...
        tm->get_topology().for_each_node([&] (const auto& node) {
            ...
            set_cell(r.cells(), "tablets_allocated", int64_t(load.get_load(host)));
            set_cell(r.cells(), "tablets_allocated_per_shard", data_value(load.get_avg_shard_load(host)));
            ...
            if (stats && stats->capacity.contains(host)) {
                ...
                auto utilization = load.get_allocated_utilization(host, *stats);
                if (utilization) {
