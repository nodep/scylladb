...
private:
    ...
    void throw_on_incomplete_data(host_id host, bool only_check_disk_capacity = false) const {
        auto node_i = _nodes.find(host);
        if (node_i != _nodes.end()) {
            auto& n = node_i->second;
            if ((n._has_all_tablet_sizes || only_check_disk_capacity) && n._has_valid_disk_capacity) {
                return;
            }
        }
...
public:
    ...
    shard_id next_shard(host_id node) {
        ...
        pick(node, shard, 1, _default_tablet_size);
        return shard;
    }
