...
public:
    ...
    void unload(host_id node, shard_id shard, int count_delta = -1, uint64_t tablet_size = service::default_target_tablet_size) {
        auto& n = _nodes.at(node);
        n.update_shard_load(shard, count_delta, tablet_size);
    }

    void pick(host_id node, shard_id shard, int count_delta = 1, uint64_t tablet_size = service::default_target_tablet_size) {
        auto& n = _nodes.at(node);
        n.update_shard_load(shard, count_delta, tablet_size);
    }
