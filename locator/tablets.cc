    ...
    if (trinfo) {
        if (trinfo->transition == tablet_transition_kind::migration) {
            ...
            if (trinfo->next.size() != ti.replicas.size()) {
                on_internal_error_noexcept(tablet_logger, ::format("Leaving and pending replica vectors for tablet in table {} have different number of elements. leaving: {} pending: {}",
